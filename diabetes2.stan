functions {
  ...
  real smoke_rf(real[] update, real constant, real female,real age_diag,real rf_prev_year, 
  real year, real ref_first, real rand){
  ...
  real micalb_rf(real[] update, real constant, real ro, real female, real age_diag, real smoker,
  real sbp, real hb1ac, real bmi, real hdl, real rand){
    real temp[size(update)];
    real predictor;
    real H1;
    real H2=H1;
    ...
    predictor=constant+age_diag*temp[2]+(sbp*temp[9])/10+female*temp[3]+hb1ac*temp[10]+
    smoker*temp[8]+bmi*temp[13]+10*hdl*temp[12];
  ...
  real efgr60l_binary(real[] update, real constant, real ro, real female, real african, 
  real asian, real age_diag, real sbp, real bmi, real hdl, real ldl, real rand){
    real temp[size(update)];
    real predictor;
    real H1;
    real H2=H1;
    ...
    predictor=constant+age_diag*temp[2]+(sbp*temp[9])/10+female*temp[3]+10*ldl*temp[11]+
    bmi*temp[13]+10*hdl*temp[12]+ asian*(temp[4]==1?1:0)+ african*(temp[4]==2?1:0);
    H1=exp(predictor)*temp[7]^(ro);
    H2=exp(predictor)*(temp[7]+1)^(ro);
    prob=1 - exp(H1 - H2);
  ...
  real[] egfr60less_rf(real[] update, real constant, real ro, real female, real african, 
  real asian, real egfr, real egfr_initial, real year){
    real temp[size(update)];
    real predictor;
    real ph1;
    real ph2;
    ...
    predictor=constant+female*temp[3]+african*(temp[4]==2?1:0)+asian*(temp[4]==1?1:0)+
    (egfr*temp[15])+ egfr_initial*temp[46]+ year*log(temp[7]);
  ...
  real[] egfr60more_rf(real[] update, real constant, real ro, real female, real african, 
  real asian, real egfr, real egfr_initial, real year){
    real temp[size(update)];
    real predictor;
    real ph2;
  ...
  real[] continuous_rf(int position_rf, real[] update,real constant,real female, real african, 
  real asian, real rf_prev_year, real age_diag, real ref_first) {
    real rf_current_year;
    real temp[size(update)];
    temp=update;
    
    rf_current_year=constant + rf_prev_year*temp[position_rf] + age_diag * log(temp[7]) + 
    ref_first*temp[position_rf + 31] + asian*(temp[4]==1?1:0)+ african*(temp[4]==2?1:0) +
    female*(temp[3]==1?1:0);
  ...
  real[] cholesterol_rf(int position_rf, real[] update,real constant,real female, real african, 
  real asian, real rf_prev_year, real age_diag, real ref_first) {
    real rf_current_year;
    real temp[size(update)];
    temp=update;
    
    rf_current_year=constant + (0.02586)*rf_prev_year*temp[position_rf] + age_diag * log(temp[7]) + 
    (0.02586)*ref_first*temp[position_rf + 31] + asian*(temp[4]==1?1:0)+ african*(temp[4]==2?1:0) +
    female*(temp[3]==1?1:0);
  ...
  real[] simCHF(real[] update, real intercept, real ro, real age_diab, real atfib, real bmi, 
  real egfr60l, real ld, real mmalb, real pvd, real amp_hist, real ulcer, real rand){
    real predictor;
    real H1;
    real H2=H1;
    real prob;
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]+bmi*temp[13]+10*ld*temp[11]+amp_hist*temp[25]+ulcer*temp[24]+
    atfib*temp[20]+(egfr60l*(temp[15]<60?temp[15]:0))/10+mmalb*temp[19]+pvd*temp[16];
    H1=exp(predictor)*temp[7]^(ro);
    H2=exp(predictor)*(temp[7]+1)^(ro);
    prob=1 - exp(H1 - H2);
    if (prob>rand){
  ...
  real[] simIHD(real[] update,real intercept,real ro,real age_diab,real female,real egfr,real hdl,
  real ldl,real pvd, real sbp, real amp_hist, real chf,  real rand){
    real predictor;
    real H1;
    real H2=H1;
    real prob;
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]+10*ldl*temp[11]+amp_hist*temp[25]+chf*temp[29]+
    (sbp*temp[9])/10+10*hdl*temp[12]+ female*temp[3]+(egfr*temp[15])/10+pvd*temp[16];
    H1=exp(predictor)*temp[7]^(ro);
    H2=exp(predictor)*(temp[7]+1)^(ro);
    prob=1 - exp(H1 - H2);
    if (prob>rand){
  ...
  real[] simMIm(real[] update,real intercept,real afro,real age_diab,real asian,real hb1ac,real hdl,
  real ldl,real mmalb,real pvd, real sbp,real smoker,real wbc,real amp,real chf,real ihd,real stroke,
  real rand){
    real predictor;
    real H1;
    real H2=H1;
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]+10*ldl*temp[11]+amp*temp[25]+chf*temp[29]+hb1ac*temp[10]+
    (sbp*temp[9])/10+10*hdl*temp[12]+smoker*temp[8]+ihd*temp[28]+stroke*temp[27]+afro*(temp[4]==2?1:0)+
    asian*(temp[4]==1?1:0)+mmalb*temp[19]+pvd*temp[16]+wbc*temp[14];
    H1=exp(predictor)*temp[7];
    H2=exp(predictor)*(temp[7]+1);
    prob=1 - exp(H1 - H2);
    if (prob>rand){
  ...
  real[] simMIf(real[] update,real intercept,real ro,real afro,real age_diab,real egfr60l,real hb1ac,
  real ldl35,real mmalb,real pvd,real sbp,real smoker,real wbc,real chf,real ihd, real rand){
    real predictor;
    real H1;
    real H2=H1;
    real prob;
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]+10*ldl35*(temp[11]>3.5?temp[11]:0)+chf*temp[29]+
    (sbp*temp[9])/10+hb1ac*temp[10]+smoker*temp[8]+ihd*temp[28]+afro*(temp[4]==2?1:0)+
    (egfr60l*(temp[15]<60?temp[15]:0))/10+mmalb*temp[19]+pvd*temp[16]+wbc*temp[14];
    H1=exp(predictor)*temp[7]^(ro);
    H2=exp(predictor)*(temp[7]+1)^(ro);
    prob=1 - exp(H1 - H2);
    if (prob>rand){
  ...
  real[] simStroke(real[] update,real intercept,real ro,real age_diab,real female,real atfib,
  real egfr60l,real hb1ac,real ldl,real mmalb,real sbp,real smoker,real wbc,real amp,real ihd,
  real rand){
    real predictor;
    real H1;
    real H2=H1;
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]+10*ldl*temp[11]+(sbp*temp[9])/10+female*temp[3]+hb1ac*temp[10]+
    smoker*temp[8]+ihd*temp[28]+atfib*temp[20]+amp*temp[25]+(egfr60l*(temp[15]<60?temp[15]:0))/10+
  ...
  real[] simStroke2(real[] update,real intercept,real ro,real age_diab,real mmalb,real smoker,
  real rand){
    real predictor;
    real H1;
    real H2=H1;
  ...
  real[] simBlind(real[] update,real intercept, real age_diab,real hb1ac, real hr, real sbp, real wbc,
  real chf, real ihd, real rand){
    real predictor;
    real H1;
    real H2=H1;
    real prob;
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]+hb1ac*temp[10]+(hr*temp[18])/10+wbc*temp[14]+(sbp*temp[9])/10+
    chf*temp[29]+ihd*temp[28];
    H1=exp(predictor)*temp[7];
    H2=exp(predictor)*(temp[7]+1);
    prob=1 - exp(H1 - H2);
    if (prob>rand){
  ...
  real pvd, real rand){
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]+hb1ac*temp[10]+pvd*temp[16]+bmi*temp[13]+
    female*temp[3];
  ...
  real[] simAmp_noUlcer(real[] update,real intercept,real ro, real age_diab, real female, real atfib,
  real hb1ac, real hdl, real hr, real mmalb, real pvd, real sbp, real wbc, real stroke, real rand){
    real predictor;
    real H1;
    real H2=H1;
    real prob;
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]++(sbp*temp[9])/10+female*temp[3]+hb1ac*temp[10]+wbc*temp[14]+
    atfib*temp[20]+10*hdl*temp[12]+(hr*temp[18])/10+mmalb*temp[19]+pvd*temp[16]+stroke*temp[27];
    H1=exp(predictor)*temp[7]^(ro);
    H2=exp(predictor)*(temp[7]+1)^(ro);
    prob=1 - exp(H1 - H2);
    if (prob>rand){
  ...
  real[] simAmp_Ulcer(real[] update,real intercept, real age_diab,real pvd,real rand){
    real predictor;
    real H1;
    real H2=H1;
    real prob;
  ...
  real[] simRenal(real[] update,real intercept, real afro,real age_diab,real female,real bmi, 
  real egfr60l,real egfr60m, real haem, real ldl,real mmalb, real sbp,real wbc,real amp, real blind, 
  real rand){
    real predictor;
    real H1;
    real H2=H1;
    ...
    temp=update;
    predictor=intercept+age_diab*temp[2]+bmi*temp[13]+10*ldl*temp[11]+amp*temp[25]+blind*temp[30]+
    +(egfr60l*(temp[15]<60?temp[15]:0))/10+(egfr60m*(temp[15]>=60?temp[15]:0))/10+female*temp[3]+
    (sbp*temp[9])/10+haem*temp[17]+mmalb*temp[19]+wbc*temp[14]+afro*(temp[4]==2?1:0);
    H1=exp(predictor)*temp[7];
    H2=exp(predictor)*(temp[7]+1);
    prob=1 - exp(H1 - H2);
    if (prob>rand){
  ...
  real[] simDeath1st(real[] update,real intercept,real asian,real year,real age,real hr,real pvd,
  real smoker,real amp_event,real ihd_event,real mi_event,real renal_event,real stroke_event,real rand){
    real predictor;
    real H1;
    real prob;
    real temp[size(update)];
    ...
    temp=update;
    predictor=intercept+year*temp[7]+age*temp[1]+smoker*temp[8]+amp_event*(temp[37]==1?1:0)+
    ihd_event*temp[34]+mi_event*temp[35]+asian*(temp[4]==1?1:0)+pvd*temp[16]+renal_event*temp[38]+
  ...
  real[] simDeath_hist(real[] update,real intercept,real phi,real bm1,real bm3,real mmalb,real smoker,
  real wbc, real amp,real chf,real renal,real stroke,real rand){
    real predictor;
    real H1;
    real H2=H1;
    real prob;
    ...
    temp=update;
    predictor=intercept+bm1*(temp[13]<18.5?temp[13]:0)+bm3*(temp[13]>25?temp[13]:0)+smoker*temp[8]+
    amp*temp[25]+chf*temp[29]+renal*temp[31]+stroke*temp[27]+mmalb*temp[19]+wbc*temp[14];
    H1=exp(predictor)*(exp(phi*(temp[1]))-1)/phi;
    H2=exp(predictor)*(exp(phi*(temp[1]+1))-1)/phi;
    prob=1 - exp(H1 - H2);
    if (prob>rand){
  ...
  real[] simDeath_sub(real[] update,real intercept,real atfib,real age,real hdl,real pvd,real wbc,
  real amp_event,real amp,real amp_event2,real ihd_event,real ihd,real mi_event,real mi,real renal,
  real stroke_event,real rand){
    real predictor;
    real H1;
    real prob;
    real temp[size(update)];
    ...
    temp=update;
    predictor=intercept+age*temp[1]+10*hdl*temp[12]+amp_event*(temp[37]==1?1:0)+amp*temp[25]+
    ihd_event*temp[34]+amp_event2*(temp[37]==2?1:0)+ihd*temp[28]+mi_event*temp[35]+mi*temp[26]+
    renal*temp[31]+stroke_event*temp[36]+atfib*temp[20]+pvd*temp[16]+wbc*temp[14];
    H1=exp(-predictor)/(1+exp(-predictor));
    prob=1 - H1;
    if (prob>rand){
      temp[23]=0;
...
generated quantities {
  ...
  for (i in 1:time){
    ...
    for (s in 1:complicaciones){
      ...
      if (order[s]==1){
        if (update[29]!= 1){
          ...
          update=simCHF(update,intercept,ro,age_diab,atfi,bmi,egf60l,ldl,mmalb,pv,amp,
          ulcer,rand);
        }
    }
    else if (order[s]==2){
      if (update[28]!=1){
          ...
          rand=uniform_rng(0,1);
          update=simIHD(update,intercept,ro,age_diab,female,efgr,hdl,ldl,pv,sbp,amp,chf,rand);
          }
    }
    else if (order[s]==3){
        if (update[26]==0){
          if (update[3]==0){
            ...
            rand=uniform_rng(0,1);
            update=simMIm(update,intercept,afro,age_diab,asian,hba1c,hdl,ldl,mmalb,pv,sbp,
            smoker,wb,amp,chf,ihd,stroke,rand);
          }
          else {
            real intercept;
            real age_diab;
        ...
        if (update[27]==0){
          ...
          rand=uniform_rng(0,1);
          update=simStroke(update,intercept,ro,age_diab,female,atfi,egf60l,hba1c,
          ldl,mmalb,sbp,smoker,wb,amp,ihd,rand);
        }
        else if (update[27]==1){
          real intercept;
          real age_diab;
        ...
        if (update[30]!=1){
          ...
          rand=uniform_rng(0,1);
          update=simBlind(update,intercept,age_diab,hba1c,heart,sbp,wb,chf,ihd,rand);
        }
      }
      else if (order[s]==6){
        real intercept;
        ...
        if(update[25]==0){
          if (update[24]==0) {
            ...
            rand=uniform_rng(0,1);
            update=simAmp_noUlcer(update,intercept,ro,age_diab,female,atfi,hba1c,
            hdl,heart,mmalb,pv,sbp,wb,stroke,rand);
          }
          else {
            real intercept;
            real age_diab;
        ...
        if (update[31]!= 1){
          ...
          update=simRenal(update,intercept,afro,age_diab,female,bmi,efr60l,efr60m,
          haemo,ldl,mmalb,sbp,wb,amp,blind,rand);
        }
      }
    }
    ...
    if(sum(update[24:31])==0 && sum(update[33:38])==0){
      ...
      rand=uniform_rng(0,1);
      update=simDeath1st(update,intercept,asian,year,age,heart,pv,smoker,amp_event,
      ihd_event,mi_event,renal_event,stroke_event,rand);
    }
    ...
    if (update[23]!=0){
      ...
      rand=uniform_rng(0,1);
      mmalb_aux=micalb_rf(update,constant,phi,female,age_diag,smoker,sbp,hb1ac,bmi,hdl,rand);
      ...
      rand=uniform_rng(0,1);
      egfr60l_binary=efgr60l_binary(update,constant,phi,female,african,asian,age_diag,sbp,bmi,
      hdl,ldl,rand);
      ...
      if(3%i==1 && i>1){
        ...
        rand=uniform_rng(0,1);
        pvd_aux=pvd_rf(update,constant,phi,age_diag,smoker,sbp,hb1ac, bmi, ldl, rand);
        ...
        rand=uniform_rng(0,1);
        smoker_aux=smoke_rf(update,constant,female,age_diag,smoker,diabet_years,smoker_first,rand);
      ...
      update=continuous_rf(10,update,constant,female,african,asian,rf_prev_value,
      ...
      update=cholesterol_rf(12,update,constant,female,african,asian,rf_prev_value,
      diabet_years,rf_first_value);
      ...
      update=cholesterol_rf(11,update,constant,female,african,asian,rf_prev_value,
      diabet_years,rf_first_value);
      ...
      update=continuous_rf(13,update,constant,female,african,asian,rf_prev_value,
      diabet_years,rf_first_value);
      ...
      if(egfr60l_binary==1){
        ...
        update=egfr60less_rf(update,constant,phi,female,african,asian,efgr,efgr_first,diabet_years);
        ...
        update=egfr60more_rf(update,constant,phi,female,african,asian,efgr,efgr_first,diabet_years);
      ...
      if(3%i==1 && i>1){
        ...
        update=continuous_rf(18,update,constant,female,african,asian,rf_prev_value,
        age_diag,rf_first_value);
        ...
        update=continuous_rf(14,update,constant,female,african,asian,rf_prev_value,
        age_diag,rf_first_value);
        ...
        update=continuous_rf(17,update,constant,female,african,asian,rf_prev_value,
        age_diag,rf_first_value);
